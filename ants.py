...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while copy_place is not hive and count < self.max_range:
            if len(copy_place.bees) == 0:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            checker = self.place.exit
            while checker:
                if checker.ant:
                    if checker.ant not in self.double:
                        checker.ant.damage = checker.ant.damage * 2
                        self.double.append(checker.ant)
                    if checker.ant.container and checker.ant.ant and checker.ant.ant not in self.double:
                        checker.ant.ant.damage = checker.ant.ant.damage * 2
                        self.double.append(checker.ant.ant)
                checker = checker.exit
            self.throw_at(self.nearest_bee(colony.hive))
